...
export default function Home() {
    ...
    useEffect(() => {
        async function initSession() {
            ...
            if (savedSessionId) {
                try {
                    // 我们借用 debug 接口把服务器上这把游戏的所有聊天记录拉回来渲染
            ...
            // 如果没有老档，或者老服务器的 Map 已经被彻底重启清除了，就重新开一局新游戏
            try {
                const res = await fetch('/api/story/start', { method: 'POST' });
                const data = await res.json();
                setSessionId(data.sessionId);
                localStorage.setItem('story_session_id', data.sessionId);
                setMessages([{ role: 'assistant', content: data.basePrompt }]);
            } catch (e) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (!res.body) throw new Error('流式读取失败');
            ...
            setMessages(prev => {
                const newMsgs = [...prev];
                newMsgs[newMsgs.length - 1].content = "网络连接中断，命运的丝线断开了...";
                return newMsgs;
            });
        } finally {
            setLoading(false);
        ...
        <div className="min-h-screen bg-[#FDFBF7] text-[#333333] font-serif flex justify-center py-6 sm:py-10 selection:bg-amber-100">
            <div className="w-full max-w-4xl flex flex-col h-[90vh] bg-white border border-[#EAE5D9] shadow-sm rounded-md overflow-hidden">
                ...
                <div className="py-5 px-6 border-b border-[#EAE5D9] bg-[#FAFAF8] text-center flex justify-between items-center relative">
                    ...
                    <div></div>
                    <h1 className="text-xl sm:text-2xl font-bold tracking-[0.3em] text-[#4A4743]">无 冬 游 侠</h1>
                    <a 
                ...
                <div ref={scrollRef} className="flex-1 overflow-y-auto p-6 sm:p-12 space-y-10 scroll-smooth custom-scrollbar">
                    {messages.map((m, i) => (
                        <div key={i} className={`flex flex-col ${m.role === 'user' ? 'items-end' : 'items-start'}`}>
                            <div 
                                className={`max-w-[90%] sm:max-w-[80%] leading-loose tracking-wide sm:text-lg ${
                                    m.role === 'user' 
                ...
                <div className="p-4 sm:p-6 bg-[#FAFAF8] border-t border-[#EAE5D9]">
                    <form onSubmit={handleSubmit} className="flex gap-4">
                        <input 
                            ...
                            placeholder={loading ? "推演中，请稍候..." : "你向何处去... (例: 走向泥泞的小径)"}
                            value={input}
